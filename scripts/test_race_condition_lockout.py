...
# Import muss in src/02_models.py sein
import importlib.util
spec = importlib.util.spec_from_file_location("models", os.path.join(project_root, "src", "02_models.py"))
models02 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(models02)
User = models02.User
...
def test_race_condition_lockout():
    ...
    # Test-DB (In-Memory für isolierte Tests)
    # In-Memory DB braucht check_same_thread=False für Threading
    engine = create_engine(
        'sqlite:///:memory:',
        connect_args={'check_same_thread': False},
        echo=False
    )
    models02.init_db(engine)
